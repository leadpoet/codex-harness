"""Thin Python entrypoint for the isolated Codex SDK bakeoff worker."""
...
_WORKER = Path(__file__).resolve().parents[1] / "codex" / "worker.mjs"
...
def run_icp(icp: dict[str, Any]) -> list[dict[str, Any]]:
    """Run one normalized ICP in one fresh Codex SDK process and thread."""

    if not isinstance(icp, dict):
        raise TypeError("icp must be a dict")
    if not _WORKER.is_file():
        raise RuntimeError(f"Codex worker is missing: {_WORKER}")
    ...
    request = {
        "protocol": "leadpoet.harness_bakeoff.codex.v1",
    ...
    try:
        ...
        raise RuntimeError("Codex SDK worker exceeded its run deadline") from exc
    payload: Any = None
    try:
        ...
        if completed.returncode != 0:
            ...
            raise RuntimeError(f"Codex SDK worker failed: {detail}") from exc
        raise RuntimeError("Codex SDK worker returned invalid JSON") from exc
    ...
    if completed.returncode != 0 or (
        ...
        raise RuntimeError(f"Codex SDK worker failed: {detail}")
